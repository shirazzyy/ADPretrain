...
from ad_models.cflow.train_val import train_val
...
def main(args):   
    ...
    config = {}
    config['action_type'] = 'norm-train'
    config['checkpoint'] = ''
    config['pool_layers'] = 3
    config['clamp_alpha'] = 1.9
    config['condition_vec'] = 128
    config['coupling_blocks'] = 8
    config['dec_arch'] = 'freia-cflow'
    config['meta_epochs'] = 25
    config['sub_epochs'] = 8
    config['dropout'] = 0.0
    config['hide_tqdm_bar'] = True
    config['lr'] = 0.0002
    ...
    for class_name in CLASS_NAMES:
        ...
        img_auc, img_ap, img_f1_score, pix_auc, pix_ap, pix_f1_score, pix_aupro = train_val(config, encoder, projectors, train_loader, test_loader,
                                                                                            ref_features[class_name] if args.with_pretrained else None,
                                                                                            class_name, args.with_pretrained)
