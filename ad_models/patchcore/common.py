...
class MeanMapper(torch.nn.Module):
    ...
    def forward(self, features):
        features = features.reshape(len(features), 1, -1)  # 将3x3的区域合并为一维向量；(dim, 3, 3) -> (dim*3*3)
        return F.adaptive_avg_pool1d(features, self.preprocessing_dim).squeeze(1) # 自适应投影到preprocessing_dim维度
...
class Aggregator(torch.nn.Module):
    ...
    def forward(self, features):
        ...
        features = F.adaptive_avg_pool1d(features, self.target_dim) # (batchsize, 1, target_dim)
        return features.reshape(len(features), -1)  # (batchsize x target_dim)
